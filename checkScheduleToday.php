<?php
	...
	$time = "";
	$schedule_today = Schedule("today", $time);

	$MD5_Schedule_today = md5($schedule_today);
	...
	if ($MD5_Schedule_today != $DB_key_today) {
		if ($schedule_today == "error") {		// Пришёл error
			if ((date("G") == 23) || (date("G") == 0)) {	// Время 23-1
				try {
					...
					$sql = "UPDATE history SET text='".$MD5_Schedule_today."', sch='' WHERE day='today'";
					$stm = $dbh->prepare($sql);
					$stm->execute($values);
			...
			try {		// Обновляем ключ и текст в БД
				...
				$sql = "UPDATE history SET text='".$MD5_Schedule_today."', sch='".$schedule_today."' WHERE day='today'";
				$stm = $dbh->prepare($sql);
				$stm->execute($values);
			...
			if ((date("G") != 23) && (date("G") != 0)) {	// Время 23-1
				$replyToday = "<b>!!! ВНИМАНИЕ !!!\nПроизошло обновление расписания пар на сегодня!</b>\n".$schedule_today;
					...
					for ($i=0; $i < count($mas_users); $i++) {		// Отправляем всем, кто разрешил отправлять
						...
						while ($row = $stmt->fetch()) {
							if ($row['notify_updates'] > 0) {
								$telegram->sendMessage([ 'chat_id' => $row['chat_id'], 'parse_mode' => 'HTML', 'disable_web_page_preview' => true, 'text' => $replyToday ]);
							}
						}
